 ...
 */
@Service
public class ZhanGuFlowFenShengRequest implements ConnectReqest {

    private final Logger logger = LoggerFactory.getLogger(ZhanGuFlowFenShengRequest.class);
    ...
    public ChargeSubmitResponse chargeRequest(ChargeRequest chargeRequest) throws Exception {
        ...
        logger.info("1、发送流程--发送供应商---战鼓分省流量--mobile ={} orderNum={}",
        ...
        try {
            logger.info("2战鼓分省流量==请求参数: {}", param.toJSONString());
            resultContent = HttpTookit.doPost(host, param.toJSONString(), "GBK");
        } catch (Exception e) {
            logger.error("3、发送异常：发送供应商---战鼓分省流量----mobile={} orderNum={} 提交到战鼓分省流量！原因：",
        ...
        logger.info("3、发送流程：发送供应商---战鼓分省流量----mobile ={} orderNum={} 返回数据：{}*******发送请求耗时：{}"
        ...
        if ("1".equals(resultObj.getString("errorCode"))) {
            ...
            logger.info("4、发送流程：发送供应商---战鼓分省流量----mobile ={} orderNum={} 提交到战鼓分省流量话费成功！",
                    chargeRequest.getMobile(),chargeRequest.getChannelNum());
        } else {
            response.setStatus(ChargeSubmitResponse.Status.FAIL);
            response.setStatusCode(resultObj.getString("errorCode"));
            response.setStatusMsg(resultObj.getString("errorDesc") + "请咨询供应商！");
            logger.info("5、发送流程：发送供应商---战鼓分省流量----mobile ={} orderNum={} 提交到战鼓分省流量失败！原因：{}请咨询供应商！",
