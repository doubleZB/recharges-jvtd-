...
/**
 * Created by liyabin on 2017/9/18.
 */
public class RongLian1013FlowRequest implements ConnectReqest {
    ...
    public ChargeSubmitResponse chargeRequest(ChargeRequest chargeRequest) {
        ...
        log.info("8、发送流程--发送供应商---容连1013流量--mobile ="+chargeRequest.getMobile()+" orderNum=" + chargeRequest.getChannelNum());
        ...
        log.info("8、发送流程---发送供应商--容连1013流量--mobile ="+chargeRequest.getMobile()+" orderNum=" + chargeRequest.getChannelNum()+" 封装应用参数json报文请求参数---" +toJson);
        ...
        try{
            ...
            log.error("8、发送异常：发送供应商---容连1013流量----mobile="+chargeRequest.getMobile()+" orderNum=" + chargeRequest.getChannelNum()+" 提交到容连1013流量！原因："+ e.getLocalizedMessage());
            response.setStatus(ChargeSubmitResponse.Status.SUCCESS);
            response.setStatusMsg(e.getLocalizedMessage()+"请咨询供应商！");
            return response;
        }
        log.info("8、发送流程----mobile ="+chargeRequest.getMobile()+" orderNum=" + chargeRequest.getChannelNum()+"  发送供应商---容连1013流量---" +toJson+"   ------返回数据：" + JSON.toJSONString(resultha)+"*******发送请求耗时："+
        ...
        if(!resultha[0].equals("failed")) {
            ...
            if (resultha[0].equals("success")) {
                if (statusCode.equals("000000")) {
                    response.setStatus(ChargeSubmitResponse.Status.SUCCESS);
                    log.info("8、发送流程：发送供应商---容连1013流量----mobile =" + chargeRequest.getMobile() + " orderNum=" + chargeRequest.getChannelNum() + " 提交容连1013流量成功！");

                } else {
                    response.setStatus(ChargeSubmitResponse.Status.FAIL);
                    response.setStatusCode(statusCode);
                    response.setStatusMsg(statusCode + "请咨询供应商！");
                    log.info("8、发送流程：发送供应商---容连1013流量----mobile =" + chargeRequest.getMobile() + " orderNum=" + chargeRequest.getChannelNum() + " 提交容连1013流量失败！原因：" + statusCode + "请咨询供应商！");
                }
                response.setStatusCode(statusCode);
                ...
                response.setChannelNum(orderNum);
                response.setSupplyChannelNum(supplyOrderNum);
                log.info("8、发送流程：发送供应商---容连1013流量----mobile =" + chargeRequest.getMobile() + " orderNum=" + chargeRequest.getChannelNum() + " 提交容连1013流量失败！原因：网络异常！");
            }
        }else{
            response.setStatus(ChargeSubmitResponse.Status.FAIL);
            response.setStatusMsg("网络异常");
    ...
    public static String[] postHttps(String myurl, String data,String datetime) {
        ...
        try {
            ...
            String str="f0fca39957b79cc80157bd03d16a0197:"+datetime;
