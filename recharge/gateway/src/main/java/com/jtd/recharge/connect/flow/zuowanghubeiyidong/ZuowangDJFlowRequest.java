 ...
 */
@Service
public class ZuowangDJFlowRequest implements ConnectReqestList {
    ...
    public ChargeSubmitResponse chargeRequest(List<ChargeRequest> chargeRequestList) throws Exception{
        ...
        for(int i=0;i<chargeRequestList.size();i++) {
            ...
            xmlDateBuffer.append("<UserPackage>").append(chargeRequest.getPositionCode()).append("</UserPackage>");
            xmlDateBuffer.append("</UserData>");
        }
        ...
        log.info("8、发送流程----mobile   发送供应商---卓望湖北移动封装应用参数json报文请求参数---" +JSON.toJSONString(bodyStr)+"   ------返回数据：" + inContent+"*******发送请求耗时："+
                (System.currentTimeMillis()-start));
        if(!"".equals(inContent)&&inContent!=null) {
            Document document = DocumentHelper.parseText(inContent);
            Element root = document.getRootElement();
            String resDesc = root.element("Response").elementText("RspDesc");
            String RspCode = root.element("Response").elementText("RspCode");
            String TransIDH = root.element("TransInfo").elementText("TransIDH");

            if ("0000".equals(RspCode)) {
                response.setStatus(ChargeSubmitResponse.Status.SUCCESS);
                response.setChannelNum(SessionID);
                response.setSupplyChannelNum(TransIDO);
                response.setSupplyOrderNum(TransIDH);
                log.info("8、发送流程：发送供应商---卓望湖北移动----mobile  提交到湖北移动成功！");
            } else {
                response.setStatus(ChargeSubmitResponse.Status.FAIL);
                response.setStatusCode(RspCode);
                response.setStatusMsg(resDesc + "请咨询供应商！");
                log.info("8、发送流程：发送供应商---卓望湖北移动----mobile  提交到卓望湖北移动失败！原因：" + resDesc + "请咨询供应商！");
