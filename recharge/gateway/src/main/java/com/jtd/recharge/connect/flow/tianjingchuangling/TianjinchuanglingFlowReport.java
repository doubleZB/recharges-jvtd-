package com.jtd.recharge.connect.flow.tianjingchuangling;
...
public class TianjinchuanglingFlowReport {
    private Log log = LogFactory.getLog(this.getClass());

    @RequestMapping("/flow/tjchuangling")
    public void report(HttpServletRequest request, HttpServletResponse response) throws IOException {

        InputStream in = request.getInputStream();
        String xml = IOUtils.toString(in);
        log.info("1.天津创领成功接收到回调，订单内容:==="+ xml);
        ...
        if("3".equals(respCode)) {
            ...
            log.info("1.天津创领回执成功===手机号==="+phoneNo+"===订单号:==="+orderID+"订单状态"+respCode);
        } else {
            report.setStatus(ChargeReport.ChargeReportStatus.FAIL);
            log.info("1.天津创领回执失败===手机号==="+phoneNo+"===订单号:==="+orderID+"订单状态"+respCode+"请咨询供应商！");
        }

        log.info("2.天津创领成功添加回执消息队列" + JSON.toJSONString(report));
        ...
        log.info("3.天津创领成功发送消息Send message id is: " + putMsg.getMessageId());
