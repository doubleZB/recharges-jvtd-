...
public class ZhanGuFlowFenShengReport {

    private final Logger logger = LoggerFactory.getLogger(ZhanGuFlowFenShengReport.class);

    @RequestMapping("/flow/zhangufs")
    public void flowRecharge(HttpServletRequest request, HttpServletResponse response) throws IOException {
        logger.info("战鼓分省流量流量回调。");
        ...
        if ("1".equals(errcode)) {
            ...
            logger.info("1.战鼓分省流量回执成功====订单状态:={}=", errcode);
        } else {
            report.setStatus(ChargeReport.ChargeReportStatus.FAIL);
            report.setMessage(errinfo + "请咨询供应商！状态" + errinfo);
            logger.info("1.战鼓分省流量回执失败===订单状态:={}=请咨询供应商！", errinfo);
        }

        logger.info("2.战鼓分省流量成功添加回执消息队列{}", JSON.toJSONString(report));
        ...
        logger.info("3.战鼓分省流量成功发送消息Send message id is:{} ", putMsg.getMessageId());
