 ...
 */
@Service
public class XuanJieFlowRequest implements ConnectReqest {
    ...
    public ChargeSubmitResponse chargeRequest(ChargeRequest chargeRequest) {
        ...
        log.info("8、发送流程：发送供应商---炫捷流量省网----mobile ="+chargeRequest.getMobile()+" orderNum=" + chargeRequest.getChannelNum());
        ...
        String nSortType = positionCode.substring(0,1);

        log.info("---packageSize ="+positionCode+" substring=" +nSortType);
        ...
        log.info("8、发送流程：发送供应商---炫捷流量省网----mobile ="+chargeRequest.getMobile()+" orderNum=" + chargeRequest.getChannelNum()+"封装应用参数json报文请求参数:"+ JSON.toJSONString(param));
        ChargeSubmitResponse response = new ChargeSubmitResponse();
        String resultContent = "";
        try {
            resultContent = HttpTookit.doPost(host,param);
        } catch (Exception e) {
            log.error("8、发送异常：发送供应商---炫捷流量省网----mobile="+chargeRequest.getMobile()+" orderNum=" + chargeRequest.getChannelNum()+" 提交到炫捷流量省网！原因："+ e.getLocalizedMessage());
            response.setStatus(ChargeSubmitResponse.Status.SUCCESS);
            response.setStatusMsg(e.getLocalizedMessage()+"请咨询供应商！");
            return response;
        }
        log.info("8、发送流程：发送供应商---炫捷流量省网----mobile ="+chargeRequest.getMobile()+" orderNum=" + chargeRequest.getChannelNum()+"返回数据：" + resultContent+"***************发送请求耗时："+
        ...
        if(code.equals("0")){
            ...
            log.info("8、发送流程：发送供应商---炫捷流量省网----mobile ="+chargeRequest.getMobile()+" orderNum=" + chargeRequest.getChannelNum()+" 提交到炫捷流量省网成功！");

        }else{
            response.setStatus(ChargeSubmitResponse.Status.FAIL);
            response.setStatusCode(code);
            response.setStatusMsg(massage+"请咨询供应商！");
            log.info("8、发送流程：发送供应商---炫捷流量省网----mobile ="+chargeRequest.getMobile()+" orderNum=" + chargeRequest.getChannelNum()+" 提交到炫捷流量省网失败！原因："+massage+"请咨询供应商！");
        }
        return response;
    }
}
