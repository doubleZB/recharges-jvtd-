 ...
 */
@Service
public class TianjingFlowRequest implements ConnectReqest{
...
//            Element root = document.getRootElement();
        ...
        try {
            String[] res= postHttps(host,xml);
            log.info("tianjinglingchuang flow getToken result---" + res[1]);
            Document document = DocumentHelper.parseText(res[1]);
            Element root = document.getRootElement();
            Token = root.element("Authorization").elementText("Token");
        } catch (Exception e) {
            log.error("tianjinglingchuang flowgetToken exception",e);
            String bool = "false";
            return bool;
        }
            log.info("tianjinglingchuang flow getToken---" + Token);
        return Token;
    }
