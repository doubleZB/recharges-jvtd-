...
<script type="text/javascript">
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                $(obj).each(function (i) {
                    ...
                    }else if(obj[i].operator==5){
                        yys="爱奇艺";
                    }else if(obj[i].operator==6){
                        yys="腾讯";
                    }else if(obj[i].operator==7){
                        yys="搜狐";
                    }else if(obj[i].operator==8){
                        yys="乐视";
                    }
                    $(productCode).each(function (j) {
                        if(obj[i].positionCode==productCode[j].code){
                            bianma=productCode[j].packageSize;
                        }
                    })
                    $(selectProvince).each(function (j) {
                        if(obj[i].provinceId==selectProvince[j].key){
                            sf=selectProvince[j].value;
                        }
                    });
    ...
    function getamountcount(){
        ...
        $.ajax({
            ...
            dataType:"json",
            data:{"mobile":model,"startime":startime,"endtime":endtime,"source":laiyuan,"provinceid":province,"operator":yunyingshang,
                "status":status,"bussinessType":bussnesstype,"userid":userid,"remark":remark,"childAccount":childAccount},
            success: function(data){
                var obj=data;
                $("#leijijinge").empty();
    ...
    function exportOrder(){
        ...
        <%--location.href="<%=request.getContextPath()%>/orderExport/exportOrder?startTime="+startTime+"&endTime="+endTime+"&businessType="+bussnesstype--%>
                <%--+"&mobile="+model+"&source="+laiyuan+"&provinceid="+province+"&operator="+yunyingshang--%>
                <%--+"&status="+status+"&remark="+remark+"&childAccount="+childAccount+"&merchantOrderNumber="+merchantOrderNumber;--%>
    }
        ...
        $.ajax({
            ...
            success: function(data){
                if(data!=null) {
                    ...
                    html += '<option value="0">无子账号</option>';
                }
                $("#doc-select-6").empty();
