 ...
 */
@Controller
@RequestMapping("/iot/cardIn")
public class CardInAction {
	private Logger logger = Logger.getLogger(this.getClass());
	@Resource
	public InReceiptService inReceiptService;
	@Resource
	...
	public String index(HttpServletRequest request) throws UnsupportedEncodingException {
		...
		request.setAttribute("statusList", InReceiptStatus.values());
		request.setAttribute("purchaseStateList", PurchaseStatus.values());
	...
	@RequestMapping("list")
	@ResponseBody
	public PageInfo<IotInReceipt> list(Integer pageNumber, Integer pageSize, IotInReceipt inReceipt) {
		PageInfo<IotInReceipt> list = inReceiptService.selectByCondition(pageNumber, pageSize, inReceipt);
		return list;
	}
	...
	@ResponseBody
	public ReturnMsg toDisplay(String purchaseSerialNum) {
		ReturnMsg msg = new ReturnMsg();
		msg.setSuccess(false);
		IotPurchase pur = purchaseService.getBySerialNum(purchaseSerialNum);
		if(pur == null) {
			msg.setMessage("该采购单不存在!");
			return msg;
		}
		IotInReceipt iotInReceipt = inReceiptService.getByPurchaseId(pur.getId());
		List<Object> ret = new ArrayList<Object>();
		ret.add(pur);
		if (iotInReceipt != null) {
			ret.add(iotInReceipt);
		}
		msg.setObject(ret);
		msg.setSuccess(true);
		return msg;
	 ...
	 * @return
	 */
	@RequestMapping("addInReceipt")
	@ResponseBody
	public ReturnMsg addInReceipt(HttpServletRequest request, IotInReceipt inReceipt) {
		ReturnMsg msg = new ReturnMsg();
		msg.setSuccess(false);
		msg.setMessage("入库成功!");
		...
		// 获得文件：
		MultipartFile importExcel = multiRequest.getFile("importExcel");
		if(importExcel == null) {
			msg.setMessage("卡信息文件未上传！");
			return msg;
		}


		IotPurchase purchase = purchaseService.getById(inReceipt.getPurchaseId());
		if (purchase == null) {
			msg.setMessage("采购单不存在!");
			return msg;
		}
		if(purchase.getPurchaseStatus() != PurchaseStatus.待入库.getValue()) {
			msg.setMessage("该采购单的状态为[" + purchase.getPurchaseStatusLiteral() + "],无法再入库!");
			return msg;
		}
		if(inReceipt.getCostDiscount() == null) {
			msg.setMessage("入库单成本折扣不能为空");
			return msg;
		}
		if(inReceipt.getCostDiscount().compareTo(purchase.getCostDiscount())>0) {
			msg.setMessage("入库单成本折扣不能高于采购单成本折扣");
			return msg;
		}
		IotProduct product = iotProductService.getProductById(purchase.getFlowProductId());
		inReceipt.setCost(product.getPrice().multiply(inReceipt.getCostDiscount()));
		AdminUser user = (AdminUser) request.getSession().getAttribute("adminLoginUser");
		try {
			inReceiptService.parseExcel(importExcel,purchase,product,inReceipt,user);
			msg.setSuccess(true);
		}  catch (Exception e) {
			logger.error(e.getMessage(),e);
			msg.setMessage(e.getMessage());
			if(!inReceipt.getInReceiptStatus().equals(InReceiptStatus.已完成.getValue())) {
				Integer count = cardService.countByInReceiptId(inReceipt.getId());
				inReceipt.setTotal(count);
				inReceipt.setCurrentInReceiptStatus(inReceipt.getInReceiptStatus());
				inReceipt.setInReceiptStatus(InReceiptStatus.失败.getValue());
				inReceiptService.changeStatus(inReceipt);
			}
		}
		return msg;
	}
	...
	public void exportPriceList(Integer inReceiptId, HttpServletResponse response, HttpServletRequest request) throws Exception {
		...
		String showFileName = iotInReceipt.getSerialNum()+ "-" + DateUtil.Date2String(new Date(), "yyyy-MM-dd");
		try {
			//创建一个Excel工作簿对象
